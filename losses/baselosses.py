...
class SSIM(nn.Module):
    ...
    def __init__(self):
        ...
        self.mu_x_pool = nn.AvgPool2d(3, 1)             #平均池化，核3*3，步幅为1
        self.mu_y_pool = nn.AvgPool2d(3, 1)
        self.sig_x_pool = nn.AvgPool2d(3, 1)
        self.sig_y_pool = nn.AvgPool2d(3, 1)
        self.sig_xy_pool = nn.AvgPool2d(3, 1)

        self.refl = nn.ReflectionPad2d(1)                     #反射填充
    ...
    def forward(self, x, y):
        ...
        sigma_x = self.sig_x_pool(x ** 2) - mu_x ** 2           #x的方差
        sigma_y = self.sig_y_pool(y ** 2) - mu_y ** 2
        sigma_xy = self.sig_xy_pool(x * y) - mu_x * mu_y         #xy的协方差
...
class CrossEntropyLoss2d(nn.Module):
    def __init__(self, weight=None, ignore_index=-100, bg_replace=None):      #weight用于加权不同损失函数
        super().__init__()
        self.ignore_index = ignore_index          #计算损失函数时，忽略目标张量中具有此索引的值，默认为-100
        self.bg_replace = bg_replace  # background replacement index                   #用于指定背景替换索引
    ...
    def forward(self, outputs, targets):
        ...
        return self.loss(nn.functional.log_softmax(outputs, dim=1), targets)   #先对output执行softmax，然后计算损失函数（负对数似然损失函数）
...
class CrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, ignore_index=-100, ignore_background=False, train_id_0=0, reduction='mean',
        ...
        assert weight is None or torch.is_tensor(weight), "weight has to be None or type torch.tensor"        #weight权重
        assert isinstance(ignore_index, int), "ignore_index has to be of type int"                              #ignore_index:忽略标签的索引值，默认为-100
        assert isinstance(ignore_background, bool), "ignore_background has to be of type bool"                  #ignore_background：是否忽略背景类别，默认为false
        ...
        assert isinstance(train_id_0, int), "train_id_0 has to be of type int"                                 #train_id：训练id的起始值，默认为0
        assert reduction in ('mean', 'sum', 'none'), "reduction only supports 'mean' (default), 'sum' and 'none'"    #reduction：损失的缩减方式，可选值为mean，sum和none，默认为mean
        assert isinstance(device, torch.device), "device has to be of type torch.device"                        #设备类型
        ...
        self.device = device
        self.loss = nn.NLLLoss(weight, ignore_index=ignore_index, reduction='none')   #负对数似然损失函数
        self.norm = norm
    ...
    def forward(self, outputs, targets):
        ...
        # care of afterwards.
        targets = torch.add(targets, -self.train_id_0)     #将目标标签转换为从0开始的连续编号
        bg = 255 - self.train_id_0           #计算背景类别的编号

        # Background treatment
        if self.ignore_background:
            targets[targets == bg] = self.ignore_index          #背景标签替换为可忽略类别
        else:
            targets[targets == bg] = outputs.shape[1] - 1       #将背景类别视为多分类任务中的最后一个类别

        # Calculate unreduced loss
        loss = self.loss(outputs, targets)           #上面实例化的NLLLoss
        ...
        if self.reduction == 'mean':
            ...
            if self.weight is not None:
                ...
                if self.ignore_background:
                    denom = torch.numel(targets) - int(torch.sum((targets == self.ignore_index).int()))      #torch.numel(targets)计算张量中的元素总数
                else:
                    denom = torch.numel(targets)
            # To avoid an exploding loss when all targets are background
            if denom == 0:          #避免所有元素都为背景
                denom = 1e-15
...
class BinaryCrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, ignore_index=-100, ignore_background=False, train_id_0=0, reduction='mean',
        ...
        self.reduction = reduction
        self.device = device
        self.loss = torch.nn.BCEWithLogitsLoss(weight, reduction='none')        #torch.nn.BCEWithLogitsLoss二分类任务的损失函数，计算二者之间的交叉熵损失
    ...
    def forward(self, outputs, targets):
        ...
        mask_pos = torch.zeros(outputs.shape)        #生成一个掩膜
        for i in range(outputs.shape[1]):
            mask_pos_ = mask_pos[:, i, ...]
            mask_pos_[targets == i] = 1               #获取与类别i相匹配的位置，设置为1
