...
import os
import cv2
from loaders.depth.validation import custom_data_loader
# Public libraries
import numpy as np
import torch
...
class Harness(object):
    ...
    def _init_validation_loaders(self, opt):
        ...
        '''
        if hasattr(opt, 'depth_test_loaders'):
            video_path = '/data/wb_project/SGDepth-master/testvideo/test.mp4'
            # 初始化
            img_height = 256
            img_width = 256
            batch_size = 16
            ...
            loader = custom_data_loader(img_height, img_width, video_path, batch_size, num_workers)

            # 调用
            self.depth_validation_loader = loader
    ...
    def _run_depth_validation(self, images_to_keep=0):
        ...
        with torch.no_grad(), self.state.model_manager.get_eval() as model:
            for frames, depths in self.depth_validation_loader:
                frame = frames[0]  # Get the first frame from the tuple
                domain = ['custom_data_loader']
                if domain not in scores:
                    scores[domain] = DepthRunningScore()
                    ratios[domain] = list()
                    images[domain] = list()
                _ = self._validate_batch_depth(model, (frames, depths), scores[domain], ratios[domain], images[domain])
        ...
        with torch.no_grad(), self.state.model_manager.get_eval() as model:
            ...
            for batch in self.depth_validation_loader:
                ...
                #print(domain)   kitti_zhou_test_depth
                if domain not in scores:
                    scores[domain] = DepthRunningScore()           #mingtian kan yixia zhege hanshu
                    ratios[domain] = list()
                    images[domain] = list()
