...
class SGDepth(nn.Module):
    ...
    def __init__(self, split_pos=1, num_layers=18, grad_scale_depth=0.95, grad_scale_seg=0.05,
        ...
        self.labels_to_replace = [0, 1, 2, 3,4, 10,19]
    ...
    def computer(self, batch):
        ...
        # Cut the stitched-together tensors along thex
        ...
        for idx, dataset in enumerate(batch):
            if self._check_purposes(dataset, 'depth'):
                ...
                with torch.no_grad():
                    ...
                    segs_pred_tensor = segs_pred_tensor.unsqueeze(1)
                    outputs[idx]['segmentation_mask',0] = segs_pred_tensor
    ...
    def forward(self,batch,epoch=1):
        outputs=self.computer(batch)
        return outputs