...
class PreConvBlock(nn.Module):
    """Decoder basic block
    """
    def __init__(self, pos, n_in, n_out):  #定义了一个预卷积块类
        super().__init__()
        self.pos = pos
...
class UpSkipBlock(nn.Module):   #定义了一个上采样跳跃连接块类
    """Decoder basic block
...
class PartialDecoder(nn.Module):
    ...
    def __init__(self, chs_dec, chs_enc, start=0, end=None):  #给定的chs_dec（解码器内部通道数）和chs_enc（来自编码器的通道数）
        super().__init__()
